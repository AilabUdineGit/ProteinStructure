...
with open(os.path.join(MODEL.corpus, 'train.txt'), 'w', encoding="utf-8") as f:
    with open("../new_17resid_train_data.txt", "r") as input_file:
        idx = 0
        for line in tqdm(input_file.readlines(), desc="creating train.txt"):
            if idx%5==0:
                line = " ".join(list(line.strip()))
                f.write(line.replace("B", "[PAD]")+"\n")
            idx+=1
            
with open(os.path.join(MODEL.corpus, 'eval.txt'), 'w', encoding="utf-8") as f:
    with open("../new_17resid_test_data.txt", "r") as input_file:
        idx=0
        for line in tqdm(input_file.readlines(), desc="creating eval.txt"):
            if idx%5==0:
                line = " ".join(list(line.strip()))
                f.write(line.replace("B", "[PAD]")+"\n")
            idx+=1
...
for item in train_dataset:
    num_tokens_train_dataset += len(item)
