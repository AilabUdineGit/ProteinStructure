...
# CORPUS.all_clean_files = "data/clean_files"
# CORPUS.CLEF = '/mnt/sda/disease-language-bert/data/CLEF-output-textified.txt'

CORPUS.train = "data/Ntrain_data.txt"
CORPUS.test = "data/Ntest_data.txt"
...
TOK.vocab_size = 5000  # 50257 for GPT-2 BBPE, 30522 for bert-base
TOK.min_frequency = 0
TOK.lower = True
...
MODEL.max_pos_emb = MAX_SEQ  # bert-base: 512
MODEL.num_attention_heads = 4  # bert-base: 12
MODEL.num_hidden_layers = 4  # bert-base: 12

# ====================================================

MODEL.pretraining.block_size = MAX_SEQ #2     170
MODEL.pretraining.mlm_prob = 0.15  # bert: 15%

MODEL.pretraining.batch_size = 256 # 16  # bert: 256
MODEL.pretraining.gradient_accumulation_steps = 8 #256 // MODEL.pretraining.batch_size

MODEL.pretraining.epochs = 50
MODEL.pretraining.save_limit = None  # None for unlimited checkpoints
MODEL.pretraining.save_steps = "2-epoch"  # -1 to never eval/save during training,
                                          # "X-epoch" to eval/save every X epochs,
                                          # any number to eval/save every X batches

# ====================================================

MODEL.name = f"maxseq_{MAX_SEQ}_bs_{MODEL.pretraining.batch_size}x{MODEL.pretraining.gradient_accumulation_steps}_ep_{MODEL.pretraining.epochs}_nhidden_{MODEL.num_hidden_layers}"